...
//3
const idk = (list) => {
    list.forEach((book)=>console.log(book.title))
}
...
const temperature = (temp) =>{
    const f = (temp*(9.0/5.0))+32
    if(f>90){
        console.log("Shorts & Undershirt")
    } else if( f>60){
        console.log("Shorts & Shirt")
    } else{
        console.log("A jacket, hoodie, etc")
    }
}
...
const avgGrade = attendance.map(
    (student)=>{
        return{
            fullName: `${student.firstName} ${student.lastName}`, letterGrade:letter(student.gpa)
        }
    }
)
//10
